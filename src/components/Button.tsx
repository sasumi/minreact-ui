...
const SpanButton = memo(function ({ children, ...props }: AnyButtonProps) {
	return (
...
const SubmitButton = memo(function ({ children, ...props }: AnyButtonProps) {
	return (
...
const PrimaryButton = memo(function ({ children, ...props }: AnyButtonProps) {
	return (
...
const NormalButton = memo(function ({ children, ...props }: AnyButtonProps) {
	props = { ...props };
...
const AnyButton = memo(function ({ tag: Tag, children, ...props }: AnyButtonProps) {
...
const Checker = memo(function ({ checked = false, onChange = () => {}, children = null, ...props }: AnyButtonProps) {
	const [chk, setChk] = useState(checked);
	...
	const onClick = () => {
		setChk(!chk);
		onChange && onChange(!chk);
	};
	return (
		<SpanButton
			className={`checker ${chk ? "checked " : ""} ${children ? "checker-has-content" : ""} ${className ? className : ""}`}
			onClick={onClick}
			{...attrs}
		>
			{children || ""}
...
const ReloadButton = ({ payload, className, resultKeepTime }: { payload: () => Promise<any>; className?: string; resultKeepTime?: number }) => {
...
export { SpanButton, AnyButton, Checker, ReloadButton, SubmitButton, PrimaryButton, NormalButton };
